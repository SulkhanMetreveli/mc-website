...
import {
  ...
  MFA_GRACE_DAYS, mfaGraceActive, mfaSatisfied, mfaStatusFor, handleMfa, trustDevice, deviceTrusted, forgetDevices,
...
export default async (req: Request) => {
  ...
  if (seg === "login" && method === "POST") {
    ...
    return json({
      ok: true, must_change_password: e.must_change_password, mfa_status: mfaStatusFor(e, trusted), grace_until: e.mfa_grace_until,
  ...
  if (seg === "mfa") {
    return handleMfa({
      ...
      account: me, accountLabel: me.work_email, issuer: "Met Capital Staff", session,
      save: () => saveEmployee(me),
      markSession: () => markSessionMfa(req),
      trust: () => trustDevice(me.id),
      forget: () => forgetDevices(me.id),
    });
  }
